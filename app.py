...
Se mantiene el 100% de la lógica original, lectura forzada de 'MUESTRA_FINAL' y exportación Word.
"""
...
st.set_page_config(
    ...
    page_icon="🏦",
    layout="wide",
    initial_sidebar_state="collapsed",  # Ocultado para simular experiencia móvil/app limpia
)
...
# Estados persistentes para simular el stepper de navegación de las pantallas
            ...
            for key, label in items_por_categoria[icon]:
                ...
                if st.button(f"{icon} {marcador_visual} {label}", key=f"btn_crit_{key}", use_container_width=True):
                    ...
                    if not is_checked:
                        ...
                        if (ahora - ultimo_clic) < 0.8: # Doble clic móvil detectado de manera segura
...
# --------------------------------------------------------------------------
# ENCABEZADO DE CONTEXTO DE CLIENTE ACTIVO (Ficha Fija Superior)
# --------------------------------------------------------------------------
    ...
    <div class="client-profile-box">
        <table style="width:100%; border:none; background:none;">
            <tr style="border:none;">
                ...
                <td>
                    <small style="color:#666; font-weight:bold;">CLIENTE SELECCIONADO</small><br>
...
# ==========================================================================
# 1️⃣ PASO: BÚSQUEDA Y CARGA (busqueda y carga.png) - Forzado Estricto a MUESTRA_FINAL
# ==========================================================================
if st.session_state.step == "Búsqueda y Carga":
    st.title("🏦 Evaluación de Carteras e Ingreso de Visitas")
    
    col1, col2 = st.columns(2)
    with col1:
        st.markdown('<div class="mockup-card">', unsafe_allow_html=True)
        st.subheader("📂 Base de Datos de la Cartera")
        filas_a_saltar = st.number_input("Filas a saltar en la cabecera del Excel:", min_value=0, value=0)
        excel_file = st.file_uploader("Subir archivo de créditos (.xlsx)", type=["xlsx", "xls"])
        
        if excel_file is not None:
            try:
                excel_lector = pd.ExcelFile(excel_file)
                # Forzado estricto e inteligente a buscar la pestaña 'MUESTRA_FINAL' solicitada
                hoja_objetivo = "MUESTRA_FINAL"
                if hoja_objetivo not in excel_lector.sheet_names:
                    hoja_objetivo = excel_lector.sheet_names[0] # Fallback de contingencia
                    st.warning(f"⚠️ No se halló la pestaña 'MUESTRA_FINAL', leyendo la primera disponible: '{hoja_objetivo}'")
    ...
    with col2:
        st.markdown('<div class="mockup-card">', unsafe_allow_html=True)
        st.subheader("🔍 Localizador del Titular")
        busq = st.text_input("Buscar por DNI, Nombre o Código del Cliente:")
        ...
        if df is not None and busq:
            ...
            if len(resultados) > 0:
                ...
                st.warning("No hay coincidencias en los datos evaluados de la hoja MUESTRA_FINAL.")
        st.markdown('</div>', unsafe_allow_html=True)
...
# ==========================================================================
# 3️⃣ PASO: VISITA (DOMICILIO/NEGOCIO) (ubicacion.png)
# ==========================================================================
elif st.session_state.step == "Visita":
    st.title("📍 Levantamiento de Información In Situ")
    
    punto_v = st.radio("Selecciona el tipo de entorno a verificar:", ["Domicilio", "Negocio", "Aval"], horizontal=True)
    ...
    # Bloque de telemetría y captura física de evidencias multimedia
    st.markdown('<div class="mockup-card">', unsafe_allow_html=True)
        ...
        if st.button("📡 Obtener Coordenadas GPS Actuales", key=f"gps_btn_{clave_v}"):
            if GEO_OK:
                ...
                if loc and "coords" in loc:
                    ...
                    st.warning("Permiso de localización denegado o no disponible.")
            else:
    ...
    # Sección complementaria de Garantías y Deudas Externas (RCC) acopladas estructuralmente a la sección de visita
...
# ==========================================================================
    ...
    # Lógica de cálculo matemático idéntica al código original
    ...
    <div style="background-color: white; border-radius:12px; padding: 1.5rem; border: 1px solid #D2D7DF; text-align:center; margin-bottom: 1.5rem;">
        ...
        <p style="margin:4px 0 0 0; color:#777; font-size:0.9rem;">Resultado Neto Total: {fmt_money(resultado_neto)}</p>
    </div>
    """, unsafe_allow_html=True)
...
# ==========================================================================
# 5️⃣ PASO: REPORTE (reporte.png) - Compilador de Salida .DOCX
# ==========================================================================
    ...
    status_aval = "🟢 Aval Completo" if "aval" in st.session_state.visitas else "⚪ Aval No Requiere"
    
    rc1.markdown(f"<div style='text-align:center;' class='{\
        'badge-ok' if 'domicilio' in st.session_state.visitas else 'badge-pend'}'>{status_dom}</div>", unsafe_allow_html=True)
    rc2.markdown(f"<div style='text-align:center;' class='{\
        'badge-ok' if 'negocio' in st.session_state.visitas else 'badge-pend'}'>{status_neg}</div>", unsafe_allow_html=True)
    rc3.markdown(f"<div style='text-align:center;' class='badge-ok'>{status_aval}</div>", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

    # MOTOR ORIGINAL DE CONSTRUCCIÓN DEL DOCUMENTO (.DOCX)
    ...
    try:
        ...
        st.error(f"Complete los pasos previos para procesar el reporte. Detalle: {e}")
    st.markdown('</div>', unsafe_allow_html=True)
